...
def getLoGImage(inputImage, inputMask, store_path, **kwargs):
  r"""
  ...
  for sigma in sigmaValues:
    ...
    if sigma > 0.0:
      if numpy.all(size >= numpy.ceil(sigma / spacing) + 1):
        ...
        sitk.WriteImage(lrgif.Execute(inputImage), store_path+'\\'+inputImageName+'.nii.gz')
...
def getWaveletImage(inputImage, inputMask, store_path, **kwargs):
  """
  ...
  for idx, wl in enumerate(ret, start=1):
    for decompositionName, decompositionImage in wl.items():
      sitk.WriteImage(decompositionImage, store_path+'\\'+decompositionName+'.nii.gz')
