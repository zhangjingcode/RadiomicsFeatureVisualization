...
from SKMRradiomics import generalinfo, getFeatureClasses, getImageTypes, getParameterValidationFiles, \
  imageoperations
...
class RadiomicsFeaturesExtractor:
  ...
  def __init__(self, *args, **kwargs):
    ...
    self._feature_map_store_path = ''
    if len(args) == 2 and isinstance(args[0], six.string_types) and os.path.isfile(args[0]):
      self.logger.info("Loading parameter file")
      self._applyParams(paramsFile=args[0])
      self._feature_map_store_path = args[1]
      ...
      print('Lack of Parameter_path or Matrix save path')
    else:
  ...
  def execute(self, imageFilepath, maskFilepath, label=None, voxelBased=False):
    ...
    for imageType, customKwargs in six.iteritems(self._enabledImagetypes):
      ...
      imageGenerators = chain(imageGenerators, getattr(imageoperations, 'get%sImage' % imageType)
      (image, mask, store_path=self._feature_map_store_path, **args))
  ...
  def computeFeatures(self, image, mask, imageTypeName, **kwargs):
    ...
    for featureClassName, enabledFeatures in six.iteritems(self._enabledFeatures):
      ...
      if featureClassName in self.getFeatureClassNames():
        ...
        try:
          featureClass = self.featureClasses[featureClassName](image, mask, self._feature_map_store_path, **kwargs)
        except:
